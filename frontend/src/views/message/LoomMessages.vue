  ...
  <div class="card mb-5 mb-xl-8">
    ...
    <div class="card-header border-0 pt-5 mb-5">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold fs-3 mb-1">Loom Conversations</span>
        <span class="fw-semibold text-muted fs-7 d-block lh-1">
          {{ threads.total }} Conversations
        </span>
      </h3>
      <div class="card-toolbar">
        <div class="me-2">
          <el-input
              v-model="threads.serverData.search.text"
          ...
          >
            <template #prepend>
              <el-button :icon="Search" @click="getThreads"/>
            </template>
            <template #append>
              <el-select
                  v-model="threads.serverData.search.type"
                  placeholder="Select"
                  style="width: 115px"
              >
        ...
        <div>
          <el-checkbox-group
              v-model="threads.serverData.filters"
              size="default"
          >
            <el-checkbox-button
        ...
        <a class="btn btn-sm btn-success ms-2" @click="getThreads">Refresh </a>
      </div>
    </div>
    <!--end::Header-->
    ...
    <div class="card-body py-3">
      ...
      <div class="table-responsive">
            ...
            <td>
              <div class="d-flex align-items-center w-100">
                  ...
                  <div class="d-flex fs-7 align-items-center">
                    ...
                    <div>
                      ...
                      >
                        ...
                        >
                        <span
                            class="badge badge-loom-sent ms-1"
                ...
                <div class="col-1 text-end">
                  <message-drop-down
                      :message-id="thread.latest_unseen_message?.id"
                      :thread-id="thread?.id"
                  />
                </div>
              </div>
        ...
        <!--end::Table-->
        <el-pagination
            :current-page="threads.serverData.page"
...
const messageActions = ref([
  {value: "Unseen messages", label: "unseen loom reply"},
]);

const actionClicked = useDebounceFn(() => getThreads(), 1000);

const changePageGetMessages = (page) => {
  threads.value.serverData.page = page;
  getThreads();
};
...
const getThreads = (withLoading = true) => {
  ...
  ApiService.post("get-loom-threads", threads.value.serverData)
      .then((response) => {
        threads.value.data = response.data.data;
        threads.value.total = response.data.total;
      })
      .finally(() => {
        loading.value = false;
      });
