...
class BrowserRegisterEmailEvent(InstagramMiddleware):
    ...
    def _click_on_profile(self):
        ...
        for selector in profile_selectors:
            try:
                self.ig.page.locator(selector).first.click(timeout=5000)
                self.ig.page.wait_for_load_state("domcontentloaded", timeout=8000)
    ...
    def _click_on_accounts_center(self):
        ...
        for selector in accounts_center_selectors:
            try:
                ...
                self.ig.page.wait_for_url("*accountscenter.instagram.com*", timeout=8000)
                self.ig.page.wait_for_load_state("domcontentloaded", timeout=8000)
                self.ig.pause(3000, 4000)
        ...
        for selector in add_email_selectors:
            try:
                ...
                if email_element.is_visible():
                    email_element.click(timeout=5000)
                    self.ig.pause(2000, 3000)
                    ...
                    try:
                        ...
                        if elements:
                            ...
                            if email_href and 'temp-mail.org/en/view/' in email_href:
                                first_email.click()
                                self.page1.wait_for_load_state("domcontentloaded", timeout=8000)
                                self.ig.pause(3000, 5000)
