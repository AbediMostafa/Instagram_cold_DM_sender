...
class LinkParser:
    """
    Parse Instagram links and extract type + username/story_id/post_code
    ...
    @staticmethod
    def is_reel(link):
        """Check if link is specifically a reel URL"""
        parsed = LinkParser.parse(link)
        return parsed.get('type') == LinkParser.TYPE_REEL

    @staticmethod
    def is_post(link):
        """Check if link is specifically a post URL"""
        parsed = LinkParser.parse(link)
        return parsed.get('type') == LinkParser.TYPE_POST
