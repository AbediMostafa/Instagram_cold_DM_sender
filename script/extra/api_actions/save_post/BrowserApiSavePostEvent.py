...
class BrowserApiSavePostEvent(BaseAction):
    ...
    def init(self):
        ...
        for i in range(batch_size):
            ...
            if not action:
                if i == 0:
                    self.ig.account.add_cli('No prepared save_post actions available')
                break
    ...
    def _handle_execution_error(self, json_data):
        ...
        3. Prepare will verify if post is still available via browser
        """
    ...
    def _log_to_file(self, message, log_type='info'):
        ...
        try:
            ...
            log_file = os.path.join(log_dir, 'api_save_post.log')
