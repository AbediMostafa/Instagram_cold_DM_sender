...
class CheckNewMessageHooks:
    ...
    def check_for_new_messages(self):
        ...
        for self.ig_thread in self.ig_threads:
            self.account.add_cli(f'Processing thread with instagram id of : {self.ig_thread.id}')

            self.get_thread_record()
            self.handle_thread_user()

            if not self.db_thread:
                self.account.add_cli("We haven't created this thread before, starting to create it ...")
                self.create_thread()
        ...
        for self.ig_message in reversed(self.ig_thread.messages):
            ...
            if self.ig_message.item_type != 'text':
                self.account.add_cli(
                    f"Current message's item type is {self.ig_message.item_type} and we're not processing it")
                continue
    ...
    def handle_thread_user(self):
        ...
        self.account.add_cli(f'This thread has {len(users)} users except me')
        self.ig_user = users[0]
        self.account.add_cli(f"And the user's username is : {self.ig_user.username}")

        self.get_user_record()

        if not self.db_user:
            self.account.add_cli("We haven't user in our database... creating it as a lead")
            self.create_lead()
